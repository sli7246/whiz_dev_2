...
define ('BASE_URL', 'http://www.whizium.com');
...
function my_error_handler ($e_number, $e_message, $e_file, $e_line, $e_vars) {
	...
	if (!LIVE) { // Development (print the error).
		...
		mail(EMAIL, 'Site Error!', $message, 'From: johnsli@msn.com');
		
		// Only print an error message if the error isn't a notice:
		if ($e_number != E_NOTICE) {
			echo '<div class="error">A system error occurred. We apologize for the inconvenience.';
			print htmlspecialchars(print_r($e_message, true));
			echo '</div><br />';
		}
	} // End of !LIVE IF.
